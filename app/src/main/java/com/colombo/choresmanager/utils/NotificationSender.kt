...
class NotificationSender(private val context: Context) {
    ...
    fun sendReminderNotification(choreId: Int, choreName: String) {
        ...
        val observable = dao.getChoreById(choreId)
        observable.observe(context as LifecycleOwner) { chore ->
            observable.removeObservers(context)
            if (chore != null) {
                val completeIntent = Intent(context, IntentReceiver::class.java).apply {
                    action = IntentActions.COMPLETE.name
                    putExtra("choreId", choreId)
                    putExtra("choreName", choreName)
                }
                val snoozeIntent = Intent(context, IntentReceiver::class.java).apply {
                ...
                val builder = NotificationCompat.Builder(context, DEFAULT_CHANNEL_ID)
                    ...
                    .addAction(
                        ...
                        ContextCompat.getString(context, R.string.complete),
                        completePendingIntent
                    )
                    .addAction(
                        R.mipmap.ic_launcher,
                        ContextCompat.getString(context, R.string.snooze_default),
                        snoozePendingIntent
                    )
